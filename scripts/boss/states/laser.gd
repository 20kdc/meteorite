...
func fixed_update(delta):
	...
	if raycast.is_colliding():
		...
		# Workaround by 20kdc: Raycast is not ignoring the parent like it should.
		# Maybe it's the platform?
		# In any case, I moved the raycast and we'll compensate here.
		length -= raycast.translation.z
